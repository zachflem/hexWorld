...
**Steel** does not use land extraction tiles. World-gen **scrap stashes** (grey ring when known, banded richness hints) are hauled by a visible **Scrapper** into a **Scrap Yard** local stockpile. Stash haul drains the hex’s shared **`remainingResource`** (same tile-based pool every other resource structure uses — [issue #28](https://github.com/zachflem/hexWorld/issues/28)); there is no separate `remainingSteel`. When the yard stockpile is full the Scrapper pauses at the yard (keeping its stash assignment) and resumes once collect or the yard courier frees room. L1 yards are manual-collect only; L2+ yards run an implied last-mile courier into the shared pool. Yard level also gates Scrapper capacity/speed (Auto next-stash at L3). Details: [Milestone26.md](Milestone26.md), [ScrapperEconomy.md](ScrapperEconomy.md).
...
- **Wandering Scout** — the land exploration unit (max one per barracks). Built for a flat resource cost equal to ten old one-shot scout train costs (300 food + 200 wood), with a build timer; no stockpile / retire gate. Once complete it wanders connected land forever, revealing every tile it visits (`engine/wanderingScouts.ts`). With **Improved Optics** researched, each step also reveals the first ring around the stepped hex (all terrain types including water — "binoculars"). With **Scout to Own** researched, every newly-scouted tile (stepped tile, plus the optics ring if Improved Optics is also complete) is also claimed as owned territory. There is no trainable one-shot scout and no manual adjacent-tile reveal — land fog clears only through Wandering Scouts (water through Scout Skiffs, § water units below).
- **Militia units** — a standing army, cheap to train, that costs ongoing food upkeep per tick for **barracks-idle** troops only (garrisoned / expedition / assault commitments skip idle upkeep). If upkeep can't be paid, an idle unit deserts (cheapest first). Militia count feeds two stats: **attack** (assault power spent two ways — claiming unowned tiles to expand territory, §6, and assaulting zombie dens, §13) and **defense** (contributes to the base's last-stand defense alongside base reinforcement HP, §9).
...
**The hidden lab** sits on a single fixed tile somewhere on the map, guarded by a permanently stationed (non-horde) defender whose strength is rolled once per world seed from a high band (`guardian_defense_min`..`max`) — tougher than anything else encountered, and different game-to-game. Watchtowers guide the search; den clears refine it; Wandering Scouts (or Scout Skiffs) can still stumble onto the lab hex without any clues.
...
- **Watchtower signals:** active watchtowers (level 2+) occasionally pick up a vague directional **signal** (coarse compass quadrant toward the lab). Signals are not clues; they lightly bias wandering-scout steps toward that sector with only a faint pull toward the true lab (`lab_clues.passive_surfacing.signal_*`). Finding the lab clears the active signal; once all den clues are collected, no further signals surface.
- **Scout reveal:** Wandering Scouts / Scout Skiffs reveal fog as usual — including the lab hex if they step it — with or without clues or a search cluster. Once the lab tile is scouted, the map draws the lab marker (same treatment as dens) and the discovery toast links its coordinates.
...
**Win condition:** locate and secure the lab — that alone ends the game in a win. Clearing dens is never required to win (a lucky scout can find the lab early), but dens are the only source of the five clues that unlock the search-cluster hint, plus outposts and army growth toward the guardian. Dispatching the lab assault elevates a ceremony HUD (top banner + viewport perimeter) for the march — see §14. See [Milestone 15](https://github.com/zachflem/hexWorld/milestone/16) / [`context/Milestone15.md`](Milestone15.md).
...
- **Tile interaction:** click any tile to see current intel (if scouted) plus available actions and their costs. Structure / den / lab / stash / tombstone **stats** live on the tile sheet’s **Info** filter (the touch-friendly path). Desktop also shows a mouse-hover tooltip for the same glance stats while the cursor is over an unselected tile.
- **Confirmations:** costs are always shown before commitment; most actions execute instantly on click; demolishing (and other destructive flows like discard save / load-replace) require an explicit in-app sheet confirmation given their permanence.
- **Top-right HUD:** a fixed column stacks **timer notifications** (builds, upgrades, repairs, training, expeditions, den assaults, recalls, sieges) and **one-off toasts** (e.g. den cleared, lab clue). Each row shows its icon on the left, label/coords, and remaining time. After **5 seconds** realtime the label/countdown slides away to the right, leaving a compact **icon peek** at the column edge; tap the icon to expand again. Active countdown rows stay peeking until the timer finishes; ephemeral toasts linger collapsed briefly then dismiss. **Exception — expedition arrival orders:** the decision row stays fully expanded (message + Redeploy / Reinforce / Garrison / Recall) until the player picks an order or the auto-recall deadline fires — no collapse-to-peek. **Exception — lab assault:** the win-condition march is not a tray row — a full-width top banner (resource bar drops below it) plus a pulsing gold viewport perimeter keep the moment front-and-center until the party arrives or is recalled. A separate **slide-out panel** (same corner) holds settings, help/rules reference, and is expected to evolve.
...
Dock-only unit:
- **Scout Skiff** — a mobile unit (capped at one per dock) that wanders randomly across its dock's connected body of water indefinitely, revealing every tile it drifts across — the water equivalent of the Wandering Scout, automatic and ongoing. With **Improved Optics** researched, each step also reveals the first ring around the stepped hex (all terrain types including land — mirroring the Wandering Scout). With **Scout to Own**, every newly-scouted tile (plus optics ring) is also claimed as owned territory, enabling cross-water footholds when combined with Improved Optics.
