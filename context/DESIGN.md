...
**Clue mechanism:**
- **Trigger:** newly revealed tiles from a Wandering Scout carry a small passive chance to surface a clue. Active watchtowers (level 2+) occasionally pick up a vague directional **signal** (always a coarse compass quadrant toward the lab) — they do not award clues themselves; the signal biases wandering scouts to investigate that sector. Clearing a den always awards exactly one clue outright. Once all clues are collected, no further clues (or watchtower signals) surface.
...
- **Precision:** a fixed total of 5 clues, narrowing down to a small cluster of hexes rather than the exact tile — the player still has to let Wandering Scouts (or Scout Skiffs on water) reveal that cluster to pinpoint the lab.
...
- **Difficulty profiles** — `public/profiles/index.json` registers shipped profiles (`default`, `hard`, …). Each folder contains `tweaks.jsonc` plus optional `assets/` (partial sprite overrides). Default art lives in `public/profiles/default/assets/` (`terrain/`, `structures/`, `resources/`, `units/`, `markers/`). Asset resolution: active profile → default profile → flat-colour fallback (`src/render/assetPaths.ts`). Structure icons also try level/tier variants before the unlevelled name ([issue #67](https://github.com/zachflem/hexWorld/issues/67) / [Milestone24.md](Milestone24.md)). `profileSlug` is persisted in IndexedDB with the save. Default terrain tiles were replaced under legacy #P12 (flat-hex sources → 256×384 via `scripts/convert-flat-terrain-hex.py`). **Still planned:** Scrapper economy (legacy #P11 / open proposed issue).
