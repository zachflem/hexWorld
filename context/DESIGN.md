...
**Clue mechanism:**
- **Trigger:** every scout action carries a small passive chance to surface a clue. Active watchtowers (level 2+) occasionally pick up a vague directional **signal** (always a coarse compass quadrant toward the lab) — they do not award clues themselves; the signal biases wandering scouts to investigate that sector, where newly scouted tiles can roll a real clue. Clearing a den always awards exactly one clue outright. Once all clues are collected, no further clues (or watchtower signals) surface.
