...
**Expanding beyond the starting 19** is mostly not automatic — the primary route is force along your footprint. In the current build this is an **expedition**: the player commits a mixed party (militia/knights/snipers) and provisions to a route through **owned ∪ scouted** ground (pathfinding prefers owned corridors). The party claims unowned scouted tiles as it crosses them (free-claim — no tile-defense wipe). A **horde** on the road is fight-or-wipe (`partyAttackPower >= hordeSize` clears it with no losses; otherwise the party is destroyed and tombstoned). Mid-march **recall** refunds unused outbound provisions pro-rata. On arrival the party waits for a short decision window with parallel options: **redeploy** (new leg from here with current troops), **reinforce** (inbound detachment from home), **garrison** (station here, cancel wait), **recall** (march home now), or **no selection** (auto-recall when the timer expires). Choices are offered on the arrival HUD notification and on the destination tile’s bottom sheet (**Party orders**). Den/lab assaults still use the legacy corridor tile-defense fight and are unchanged. Scouted fog still matters because routes cannot cut through dark tiles.
...
**Clue mechanism:**
- **Trigger:** newly revealed tiles from a Wandering Scout carry a small passive chance to surface a clue (not gated on a watchtower signal). Active watchtowers (level 2+) occasionally pick up a vague directional **signal** (always a coarse compass quadrant toward the lab) — they do not award clues themselves; the signal biases wandering-scout movement toward that sector. Clearing a den always awards exactly one clue outright. Once all clues are collected, no further clues (or watchtower signals) surface.
- **Form:** each clue is a directional hint relative to the base — early clues give a coarse compass quadrant ("something calls from the north"), later clues refine that into a narrower arc ("north-north-east"). Watchtower signals use the same coarse quadrant language but prompt sending scouts rather than counting as a collected clue.
- **Precision:** a fixed total of 5 clues, narrowing down to a small cluster of hexes rather than the exact tile — once all clues are in, the map highlights that cluster (scouted tiles: light player-color tint; unscouted tiles: slightly reduced fog) via a jittered search-zone center (`labSearchZoneCenter`); the player still has to let Wandering Scouts (or Scout Skiffs on water) reveal the true lab tile inside it.
...
**Win condition:** locate and secure the lab — that alone ends the game in a win. Clearing dens is never required; it's simply one of the best routes to an army strong enough to beat the guardian (and a guaranteed clue source along the way). See [Milestone 15](https://github.com/zachflem/hexWorld/milestone/16) / [`context/Milestone15.md`](Milestone15.md).
