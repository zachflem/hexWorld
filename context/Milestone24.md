# Milestone 24 — Per-level structure sprites

**GitHub:** https://github.com/zachflem/hexWorld/issues/67 (legacy `#P13`)

Level/tier-aware structure sprite resolution with missing-file fallback, shipped art pack, and per-variant map placement.
...
- `context/Milestone24.md` (this file)

**Touched:**
- `src/render/tileTextures.ts` — `getStructureIconTextureCandidates`
- `src/render/HexCanvas.tsx` — level/tier-aware map draw
- `src/render/structurePlacement.ts` — per-variant scale/offset table
- `src/ui/GameScreen.tsx` — `StructureIcon` with `<img onError>` candidate walk
- `context/DESIGN.md`, `context/TWEAKS.md`, `context/attribution.md`
- `public/profiles/default/assets/structures/` — level/tier art pack

## Verification

- `npm test` / `npm run build`
- Preview on `krunchee` then merge to `dev`
