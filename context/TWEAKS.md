...
- **Superseded again by corridor conquest (#73, 2026-07-24):** territory expeditions no longer fight `tileDefense` per hex. Unowned scouted tiles on the route are **free-claimed**. Wipe risk is **hordes only** (fight if `partyAttackPower >= hordeSize`, else full wipe + tombstone). Pathfinding stays owned∪scouted and prefers owned tiles via `expeditions.unowned_path_penalty`. Arrival awaits orders for `arrival_decision_minutes` with parallel options: redeploy, reinforce (`reinforce_cost_multiplier`), garrison here, recall now, or no selection → auto-recall. The decision HUD row stays fully expanded (with action buttons) until an order is chosen or auto-recall fires (not a timed expand / peek). Mid-march recall/redeploy: **no food refund** — outbound provisions stay sunk; redeploy fully re-quotes from the current hex (ScrapperEconomy Q41/Q45). Den/lab assaults keep the legacy corridor `tileDefense` fight (`ASSAULT_CORRIDOR`) and support mid-march **recall only** (no redeploy). Assault provisions use `assault_provisions_multiplier` (0.1) on the normal expedition food formula — full rates × lab distance × win-condition armies were storage-impossible (~30k food); discounted marches land around a few thousand.
...
**Total clues:** 5, fixed (`lab_clues.total_clues`) — **den-clear awards only**. Each clue is a directional hint relative to base — early clues give a coarse compass quadrant ("something calls from the north"), later clues refine that into a narrower arc. Wandering scouts never increment `cluesCollected`.

**Final search cluster:** once all 5 den clues are collected, the map highlights a jittered cluster of radius `final_search_area_radius_tiles` (6) around `labSearchZoneCenter` (never the exact lab tile). Scouted tiles in the cluster get a light player-color wash (~10% opacity); unscouted heavy/light/hidden fog in the cluster is eased by ~10% opacity so the area peeks through. Wandering Scouts / Scout Skiffs still have to reveal the true lab hex (with or without the cluster). Cleared when the lab is secured.
