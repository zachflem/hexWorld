...
- **Scouting → Improved Optics** — wandering scouts and scout skiffs reveal the stepped tile plus its first ring (up to 7 tiles; all terrain types — "binoculars"); territory expeditions free-claim unowned neighbors of every tile they step on (including water; skips dens and the lab), so shoreline water can be owned from the adjacent land hex.
- **Scouting → Scout to Own** — wandering scouts and scout skiffs **claim** (own) every tile they newly scout, not just reveal it. Combines with Improved Optics: if both are researched, the full optics ring is also owned. No prerequisite — can be researched independently of Improved Optics. Horde-occupied tiles, active dens, and the unsecured lab are never claimed. Already-scouted tiles are not bulk-converted; they become owned only when a scout or skiff re-covers them.
