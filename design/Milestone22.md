...
2. **Registration** — name / inline colour wheel; **Show Advanced Options** (collapsed by default) holds difficulty profile, optional seed, and recent seeds (Milestone 23). Same validation logic and defaults (`DEFAULT_COLOR = "#863bff"`), reframed as a manual page. Primary button: "Continue", disabled until name is non-empty (same gate as the old submit button).
...
4. Existing save (from step 1) → reload → **continue prompt** appears first; **Yes!** resumes `GameScreen` without re-reading the manual; **No. Start a New Game** clears save and shows onboarding.
5. Emulate `prefers-reduced-motion: reduce` → pages swap instantly, no fade, still fully functional.

## Post-M22 addendum (Milestone 23, same `goblin` branch)

- Difficulty profiles + URL slugs (`/hard`, etc.) and bundled `public/profiles/{slug}/`.
- Continue-or-new-game prompt (`src/ui/ContinueGamePrompt.tsx`) — no silent auto-resume.
- Registration advanced block: difficulty, seed, recent seeds (last 5 in IndexedDB).
- Default sprites moved to `public/profiles/default/assets/` (formerly `public/tiles/`).
