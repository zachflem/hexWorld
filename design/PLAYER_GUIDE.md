...
Each resource has its own **storage cap** at your base (1000 at level 1, doubling per storage upgrade level — upgraded per-resource from the base tile sheet’s **Storage** tab; this is separate from the global tech tree described under [Research](#research)). The base sheet’s **Info** tab shows how full each stockpile is against its current cap.
